...
thread_local! {
    pub static CURRENT_MODULE: RefCell<Option<SimpleJITModule>> = RefCell::new(None);
}
...
pub(super) fn run_jit(tcx: TyCtxt<'_>) -> ! {
    ...
    jit_builder.symbols(load_imported_symbols_for_jit(tcx));
    ...
    let (mut jit_module, unwind_context) = super::time(tcx, "codegen mono items", || {
